...
public sealed class Mediator(IServiceProvider serviceProvider) : IMediator
{
    ...
    public Task<TResponse> Send<TResponse>(in IRequest<TResponse> request, CancellationToken cancellationToken)
    {
        ...
        var wrapperType = typeof(IRequestHandler<,>).MakeGenericType(requestType, typeof(TResponse));
        var handler = (IRequestHandlerBase<TResponse>)serviceProvider.GetRequiredService(wrapperType);
        return handler.Handle(request, cancellationToken);
    }
}
